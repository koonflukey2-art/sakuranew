import { PrismaClient, Role, AdPlatform, CampaignStatus } from "@prisma/client";
...
async function main() {
  ...
  // ลบข้อมูลเก่า (ถ้าเพิ่ง migrate reset มาก็ไม่มีอะไรอยู่ แต่เผื่อไว้)
  ...
  // 1. Create Users (ผูกกับ organizationId)
  ...
  // 2. Create Products (ใช้ organizationId แทน userId)
  ...
  // 3. Create Ad Campaigns (ใช้ AdPlatform ใหม่ + organizationId, ไม่มี SHOPEE / LAZADA แล้ว)
  ...
  const campaigns = await Promise.all([
    ...
    // เดิม SHOPEE → ใช้ GOOGLE แทน (แค่ platform enum)
    prisma.adCampaign.create({
      data: {
  ...
  // 4. Create Budgets (ใช้ organizationId แทน userId)
  ...
  // 5. Create Automation Rules (ยังใช้ userId = admin.id ตาม schema)
  ...
  // 6. Create Metrics Plans (ยังผูกกับ userId)
