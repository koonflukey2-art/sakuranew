...
export async function GET(request: NextRequest) {
  ...
  if (from) orderDateFilter.gte = from;
  if (to) orderDateFilter.lte = to;
  ...
  where.AND = [
    ...(where.AND ?? []),
    { OR: [{ orderDate: orderDateFilter }, { createdAt: createdAtFilter }] },
  ];
...
export async function PUT(request: NextRequest) {
  try {
    const user = await getCurrentUser(request);
    console.log("[/api/orders] PUT token:", user?.id, user?.role);
    ...
    console.log("[/api/orders] PUT dbRole:", dbUser?.role);

    if (!dbUser?.role || !["ADMIN", "STOCK"].includes(dbUser.role)) {
    ...
    if (
      existing.customerId &&
      (customerName || customerPhone || customerAddress !== undefined)
    ) {
      await prisma.customer.update({
    ...
    const nextStatus =
      statusRaw ? parseOrderStatus(statusRaw, existing.status as any) : undefined;

    const notes = body.notes !== undefined ? safeString(body.notes, "").trim() : undefined;
    const cancelReason = safeString(body.cancelReason, "").trim();
    const shouldCancel = nextStatus === "CANCELLED";
    const cancelNote = shouldCancel && cancelReason ? `ยกเลิก: ${cancelReason}` : null;

    const nextNotes =
      cancelNote !== null
        ? [notes ?? existing.notes ?? null, cancelNote].filter(Boolean).join(" | ")
        : notes;
    ...
    const order = await prisma.order.update({
      ...
      data: {
        ...
        ...(nextNotes !== undefined ? { notes: nextNotes || null } : {}),
        ...(shouldCancel
          ? { cancelledAt: new Date(), cancelledByUserId: user.id }
...
export async function DELETE(request: NextRequest) {
  try {
    const user = await getCurrentUser(request);
    console.log("[/api/orders] DELETE token:", user?.id, user?.role);
    ...
    console.log("[/api/orders] DELETE dbRole:", dbUser?.role);
