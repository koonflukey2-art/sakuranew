...
export async function GET(request: NextRequest) {
  const orderTime = (o: { orderDate: Date | null; createdAt: Date }) => (o.orderDate ?? o.createdAt);
try {
    ...
    const todayOrders = await prisma.order.findMany({
      where: {
        ...
        status: { in: ["PENDING","CONFIRMED","COMPLETED"] }, // ✅ ถ้าไม่อยากกรอง status ให้ลบบรรทัดนี้ออก
        OR: [
          { orderDate: { gte: todayStartUtc, lt: tomorrowStartUtc } },
    ...
    const weekOrders = await prisma.order.findMany({
      where: {
        ...
        status: { in: ["PENDING","CONFIRMED","COMPLETED"] }, // ✅ ถ้าไม่อยากกรอง status ให้ลบบรรทัดนี้ออก
        OR: [
          { orderDate: { gte: weekStartUtc, lt: tomorrowStartUtc } },
          { createdAt: { gte: weekStartUtc, lt: tomorrowStartUtc } },
        ],
      },
      select: {
        productType: true,
    ...
    const budgets = await prisma.capitalBudget.findMany({
      ...
      select: { amount: true, remaining: true, minThreshold: true, createdAt: true },
      orderBy: { createdAt: "desc" },
    });

    const budgetTotal = budgets.reduce((sum, b) => sum + (b.amount || 0), 0);
    const budgetRemaining = budgets.reduce((sum, b) => sum + (b.remaining || 0), 0);
    const budgetUsed = Math.max(budgetTotal - budgetRemaining, 0);
    const budgetMinThreshold = (budgets[0]?.minThreshold ?? 0);
    ...
    return NextResponse.json({
      ...
      budget: {
        ...
        minThreshold: budgetMinThreshold,
      },
    });
  } catch (error: any) {
