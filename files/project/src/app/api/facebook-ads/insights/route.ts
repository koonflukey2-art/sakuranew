...
const adAccountId = searchParams.get("adAccountId");
    ...
    const adAccount = await prisma.adAccount.findFirst({
      where: { id: adAccountId, organizationId: orgId },
    });

    if (!adAccount || adAccount.organizationId !== orgId) {
    ...
    if (adAccount.accessToken) {
      ...
      const platformCred = await prisma.platformCredential.findUnique({
        where: {
          organizationId_platform: { organizationId: orgId, platform: "FACEBOOK_ADS" },
        },
      });
