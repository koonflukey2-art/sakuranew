...
export async function POST(request: NextRequest) {
  try {
    const currentUserData = await getCurrentUser(request);
    if (!currentUserData) {
...
async function getSystemContext(organizationId: string) {
  ...
  const totalSpent = budgets.reduce((sum, b) => sum + (b.totalAmount - b.remaining), 0);
  ...
  return {
    ...
    budgets: budgets.map((b) => ({
      purpose: (b.description ?? b.name ?? null),
    ...
    alerts: {
      ...
      overBudget: budgets
          ...
          .map((b) => (b.description ?? b.name ?? null)),
    },
  };
}
