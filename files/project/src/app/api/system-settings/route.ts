  ...
  try {
    ...
    if (!settings) {
      return NextResponse.json({
        ...
        // flags for UI (do not expose secrets)
        lineChannelAccessTokenSet: false,
        lineChannelSecretSet: false,
        adsLineChannelAccessTokenSet: false,
        adsLineChannelSecretSet: false,
    ...
    return NextResponse.json({
      ...
      // flags for UI (do not expose secrets)
      lineChannelAccessTokenSet: Boolean(settings.lineChannelAccessToken),
      lineChannelSecretSet: Boolean(settings.lineChannelSecret),
      adsLineChannelAccessTokenSet: Boolean(settings.adsLineChannelAccessToken),
      adsLineChannelSecretSet: Boolean(settings.adsLineChannelSecret),
    });
  } catch (e: any) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Re-fetch sanitized response (avoid returning secrets)
    const fresh = await prisma.systemSettings.findUnique({
      where: { organizationId: orgId },
      select: {
        organizationId: true,
    ...
    return NextResponse.json({
      ...fresh,
      lineChannelAccessToken: maskSecret(fresh?.lineChannelAccessToken ?? null),
      lineChannelSecret: maskSecret(fresh?.lineChannelSecret ?? null),
      adsLineNotifyToken: maskSecret(fresh?.adsLineNotifyToken ?? null),
      adsLineChannelAccessToken: maskSecret(fresh?.adsLineChannelAccessToken ?? null),
      adsLineChannelSecret: maskSecret(fresh?.adsLineChannelSecret ?? null),

      lineChannelAccessTokenSet: Boolean(fresh?.lineChannelAccessToken),
      lineChannelSecretSet: Boolean(fresh?.lineChannelSecret),
      adsLineChannelAccessTokenSet: Boolean(fresh?.adsLineChannelAccessToken),
      adsLineChannelSecretSet: Boolean(fresh?.adsLineChannelSecret),
    });
  } catch (e: any) {
