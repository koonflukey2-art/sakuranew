...
export async function POST(request: NextRequest) {
  try {
    ...
    const config = await prisma.aIProvider.findFirst({
      where: {
    ...
    const systemContext = await getSystemContext(orgId, dbUser.id);
...
async function getSystemContext(orgId: string, userId: string) {
  ...
  const totalBudget = budgets.reduce((sum, b) => sum + b.totalAmount, 0);
    ...
    const totalBudgetSpent = totalBudget - budgetRemaining;
  ...
  return {
    ...
    budgets: budgets.map(b => ({
        ...
        spent: b.totalAmount - b.remaining,
    ...
    alerts: {
      ...
      overBudget: budgets
        .filter(b => (b.totalAmount - b.remaining) > b.totalAmount)
        .map(b => ({
          purpose: (b.name || b.description || "N/A"),
          budget: b.totalAmount,
          spent: (b.totalAmount - b.remaining),
          overBy: (b.totalAmount - b.remaining) - b.totalAmount,
        })),
