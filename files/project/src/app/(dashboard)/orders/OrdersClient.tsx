...
export default function OrdersPage() {
  const rawFetch: typeof fetch = (...args) => (globalThis.fetch as any)(...args);
  const apiFetch = async (input: RequestInfo | URL, init: RequestInit = {}): Promise<Response> => {
    const res = await rawFetch(input, { credentials: "include", ...init });
    if (res.status === 401 && typeof window !== "undefined") {
      window.location.href = "/auth/sign-in?from=%2Forders";
      return res;
    }
    return res;
  };

  const debugRes = async (res: Response) => {
    const ct = res.headers.get("content-type") || "";
    let body: any = null;
    try { body = ct.includes("application/json") ? await res.clone().json() : await res.clone().text(); } catch {}
    console.log("[orders api]", { url: res.url, status: res.status, body });
    return body;
  };
...
fetchUserRole();
  ...
  const fetchProductTypes = async () => {
    try {
      const res = await apiFetch("/api/products/types");
      if (res.ok) {
        const data = await res.json();
...
case "3d":
  ...
  const handleRefresh = () => fetchOrders({ resetPage: true });const handleUpdateOrder = async () => {
    ...
    try {
      ...
      const errBody = await debugRes(res);
  ...
  const handleDeleteOrder = async () => {
    ...
    try {
      setSubmitting(true);
      const res = await apiFetch(`/api/orders?id=${selectedOrder.id}`, {
        method: "DELETE",
      });
      if (res.ok) {
        toast({
