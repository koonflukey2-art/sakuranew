...
interface SystemSettings {
  ...
  // Stock LINE (do not display secrets; use flags instead)
...
export default function SystemSettingsPage() {
  ...
  // Flags to show "saved" status without exposing secrets
  ...
  // Auto-generate webhook URL  // ✅ Auto-generate webhook URL (Stock)
  const webhookUrl =
  ...
  // ✅ Auto-generate webhook URL (Ads)  const adsWebhookUrl =
    typeof window !== "undefined"
      ? `${window.location.origin}/api/webhooks/line-ads`
      : settings.adsLineWebhookUrl ||
        "https://your-domain.com/api/webhooks/line-ads";
  ...
  const fetchSettings = async () => {
    try {
      ...
      setTokenStatus({
        lineAccess: Boolean(data.lineChannelAccessTokenSet),
        lineSecret: Boolean(data.lineChannelSecretSet),
        adsAccess: Boolean(data.adsLineChannelAccessTokenSet),
        adsSecret: Boolean(data.adsLineChannelSecretSet),
      });
  ...
  const handleSaveSettings = async () => {
    try {
      ...
      const payload: any = {
        ...
        // ✅ Target for Push (ใช้แทน LINE Notify)
      ...
      // Stock LINE tokens (ใส่เฉพาะตอนกรอกมา)
      ...
      // ✅ Ads LINE tokens (ไม่มี LINE Notify Token แล้ว)
      if ((settings.adsLineChannelAccessToken || "").trim()) {
        payload.adsLineChannelAccessToken = (settings.adsLineChannelAccessToken || "").trim();
      }
      if ((settings.adsLineChannelSecret || "").trim()) {
        payload.adsLineChannelSecret = (settings.adsLineChannelSecret || "").trim();
      }
      ...
      setSettings((prev) => ({
        ...prev,
        dailyCutOffHour: (data as any).dailyCutOffHour ?? prev.dailyCutOffHour,
        dailyCutOffMinute: (data as any).dailyCutOffMinute ?? prev.dailyCutOffMinute,

        lineWebhookUrl: (data as any).lineWebhookUrl || webhookUrl,
        adsLineWebhookUrl: (data as any).adsLineWebhookUrl || adsWebhookUrl,

        // ✅ ไม่ให้ targetId หายหลังเซฟ
        lineTargetId: (data as any).lineTargetId ?? prev.lineTargetId,

        adminEmails: (data as any).adminEmails ?? prev.adminEmails,
        notifyOnOrder:
          typeof (data as any).notifyOnOrder === "boolean"
            ? (data as any).notifyOnOrder
            : prev.notifyOnOrder,
        notifyOnLowStock:
          typeof (data as any).notifyOnLowStock === "boolean"
            ? (data as any).notifyOnLowStock
            : prev.notifyOnLowStock,
        notifyDailySummary:
          typeof (data as any).notifyDailySummary === "boolean"
            ? (data as any).notifyDailySummary
            : prev.notifyDailySummary,

        // เคลียร์ token ใน UI (API มักจะส่ง masked/null)
lineChannelAccessToken: "",
        lineChannelSecret: "",
        adsLineChannelAccessToken: "",
        adsLineChannelSecret: "",
  ...
  const fetchProviders = async () => {
    try {
      ...
      const data = await response.json();
        ...
        const providersArray: AIProvider[] = Array.isArray((data as any)?.providers)
  ...
  const fetchPlatformCreds = async () => {
    try {
      ...
      if (res.ok) {
        ...
        const list: PlatformCredential[] = Array.isArray((data as any)?.credentials)
  ...
  const fetchAdAccounts = async () => {
    try {
      ...
      if (res.ok) {
        ...
        const list: AdAccount[] = Array.isArray((data as any)?.accounts)
        ...
        <TabsContent value="line">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                <Label>LINE Channel Access Token {tokenStatus.lineAccess && <Badge className="ml-2">ตั้งค่าแล้ว</Badge>}</Label>
              ...
              <div>
                <Label>LINE Channel Secret {tokenStatus.lineSecret && <Badge className="ml-2">ตั้งค่าแล้ว</Badge>}</Label>
                <div className="flex gap-2">
                  <Input
        ...
        <TabsContent value="line-ads">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                <Label>LINE Channel Access Token (Ads) {tokenStatus.adsAccess && <Badge className="ml-2">ตั้งค่าแล้ว</Badge>}</Label>
              ...
              <div>
                <Label>LINE Channel Secret (Ads) {tokenStatus.adsSecret && <Badge className="ml-2">ตั้งค่าแล้ว</Badge>}</Label>
                <div className="flex gap-2 mt-1">
                  <Input
