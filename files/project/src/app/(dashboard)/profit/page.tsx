...
export default function ProfitCalculatorPage() {
  ...
  const [
          hasAccess, setHasAccess] = useState<boolean | null>(null);
...
fetch("/api/profit/summary"),
        ...
        const statementsData = statementsResponse.ok ? await statementsResponse.json() : null;
        const adsData = adsResponse.ok ? await adsResponse.json() : null;

        const adsFromStatements = toNumber(statementsData?.totalAmount);
        const adsFromReceipts = toNumber(adsData?.totalAmount);

        // ✅ ใช้สเตทเมนต์เป็นหลัก ถ้ามีข้อมูล (ไม่งั้น fallback ไปสลิป)
        setAdsSpend(adsFromStatements > 0 ? adsFromStatements : adsFromReceipts);
const capitalData = capitalBudgetResponse.ok
          ? await capitalBudgetResponse.json()
          : null;

        const capitalTotal = toNumber((capitalData as any)?.totalAmount ?? 0);

        // used อาจมาจาก field used โดยตรง หรือคำนวณจาก totalAmount-available
        const usedFromApi = (capitalData as any)?.used;
        const capitalUsed = Number.isFinite(Number(usedFromApi))
          ? Number(usedFromApi)
          : Math.max(
              0,
              toNumber((capitalData as any)?.totalAmount) -
                toNumber((capitalData as any)?.available)
            );
const budgets = budgetsResponse.ok ? await budgetsResponse.json() : [];
        const safeBudgets = Array.isArray(budgets) ? budgets : (Array.isArray((budgets as any)?.body) ? (budgets as any).body : []);
        const operationsTotal = safeBudgets.reduce(
          (sum: number, budget: { totalAmount?: number; amount?: number }) =>
            sum + (toNumber(budget.totalAmount) || toNumber(budget.amount)),
          0
        );
        const operationsUsed = safeBudgets.reduce(
        ...
        const budgetRequests = budgetRequestsResponse.ok
          ...
          : [];
        const safeBudgetRequests = Array.isArray(budgetRequests) ? budgetRequests : (Array.isArray((budgetRequests as any)?.body) ? (budgetRequests as any).body : []);
        const approvedRequests = safeBudgetRequests.reduce(
              ...
              <Card className="border border-white/10 bg-white/5 lg:col-span-2">
                ...
                <CardContent className="space-y-3 text-sm text-slate-200">
                  <div className="flex items-center justify-between">
                    <span>ค่าโฆษณา (จากสเตทเมนต์)</span>
                ...
                <tbody>
                  {summary.products.map((product) => (
                    <tr key={product.id} className="border-b border-white/5">
                      ...
                      <td className="py-3 text-right">{formatCurrency(product.netRevenue ?? 0)}</td>
