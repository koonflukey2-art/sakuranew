...
const F = {
  ...
  formatCurrency: (val: unknown) => `${toNum(val).toLocaleString('th-TH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ฿`,
  formatNumber: (val: unknown, digits: number = 2) => toNum(val).toLocaleString('th-TH', { minimumFractionDigits: digits, maximumFractionDigits: digits }),
  formatInt: (val: unknown) => Math.ceil(toNum(val)).toLocaleString('th-TH'),
};
...
export function ProfitPilotPage() {
  ...
  const [n8nWorkflow, setN8nWorkflow] = useState<{ json: string | null; loading: boolean }>({ json: null, loading: false });
  ...
  const computeMetrics = useCallback((inputData: Record<string, unknown>) => {
    const newInputs = { ...(inputData as Record<string, unknown>) };
    ...
    const funnelPlanKey =
      typeof newInputs.funnelPlan === "string" && (newInputs.funnelPlan in funnelPlans)
        ? (newInputs.funnelPlan as keyof typeof funnelPlans)
        : "launch";
    const funnelPlan = funnelPlans[funnelPlanKey];
  ...
  const handleInputChange = useCallback((key: keyof typeof inputs | string, value: unknown) => {
  ...
  const platformReport = useMemo(() => {
    ...
    return Object.entries(platformFees).map(([key, fees]) => {
      ...
      return {
        key,
        name: (platformFeeLabels as Record<string, string>)[key] || key,
        fees: {
  ...
  const calculateAll = useCallback(() => {
    ...
    if (shouldUpdateInputs) {
      setInputs(prev => ({ ...prev, ...(updatedInputs as Partial<typeof prev>) }));
    }
  ...
  const handlePlatformChange = (value: keyof typeof platformFees) => {
  ...
  const addRule = () => {
    const automationToolKey =
      (typeof inputs.automationTool === "string" && (inputs.automationTool in automationToolsConfig))
        ? (inputs.automationTool as keyof typeof automationToolsConfig)
        : "facebook";
    const toolConfig = automationToolsConfig[automationToolKey];
    const newRule = {
      id: Date.now(),
      name: '',
  ...
  const updateRule = (id: number, field: string, value: any) => {
    setAutomationRules(prev => prev.map((r: any) => r.id === id ? { ...r, [field]: value } : r));
  };
  
  const deleteRule = (id: number) => {
    setAutomationRules(prev => prev.filter((r: any) => r.id !== id));
  };
  ...
  const fetchAiAdvice = useCallback(async () => {
    ...
    try {
      ...
      const advice = await getMetricsAdvice({
        businessType: (typeof inputs.businessType === "string" && (inputs.businessType in funnelObjectivesData) ? funnelObjectivesData[inputs.businessType as keyof typeof funnelObjectivesData].name : inputs.businessType),
        profitGoal: F.num(inputs.profitGoal),
        fixedCosts: F.num(inputs.fixedCosts),
        ...
        funnelPlan: (typeof inputs.funnelPlan === "string" && (inputs.funnelPlan in funnelPlans) ? funnelPlans[inputs.funnelPlan as keyof typeof funnelPlans].name : inputs.funnelPlan),
        metricsPlan: (typeof inputs.metricsPlan === "string" && (inputs.metricsPlan in metricsPlans) ? metricsPlans[inputs.metricsPlan as keyof typeof metricsPlans].name : inputs.metricsPlan),
      });
  ...
  const metricsPlanKey =
    (typeof inputs.metricsPlan === "string" && (inputs.metricsPlan in metricsPlans))
      ? (inputs.metricsPlan as keyof typeof metricsPlans)
      : "fb_s1_plan";
  const selectedMetricsPlan = metricsPlans[metricsPlanKey];
  ...
  const businessTypeKey =
    (typeof inputs.businessType === "string" && (inputs.businessType in funnelObjectivesData))
      ? (inputs.businessType as keyof typeof funnelObjectivesData)
      : "ecommerce_website_campaign";
  const funnelObjectives = funnelObjectivesData[businessTypeKey].objectives;
  ...
  const currentFunnelPlanKey =
    typeof inputs.funnelPlan === "string" && (inputs.funnelPlan in funnelPlans)
      ? (inputs.funnelPlan as keyof typeof funnelPlans)
      : "launch";
  const currentFunnelPlan = funnelPlans[currentFunnelPlanKey] || { tofu: 0, mofu: 0, bofu: 0 };
  ...
  const funnelLabels = useMemo(() => {
    const funnelPlanKey =
      typeof inputs.funnelPlan === "string" && (inputs.funnelPlan in funnelPlans)
        ? (inputs.funnelPlan as keyof typeof funnelPlans)
        : "super_launch";
    const plan = funnelPlans[funnelPlanKey] || { tofu: 0, mofu: 0, bofu: 0 };
    return {
  ...
  const FloatingIcon = ({ icon, className = "", size = "md", style = {} }: { icon: any; className?: string; size?: "sm" | "md" | "lg"; style?: CSSProperties }) => {
  ...
  const FunnelStructure = ({ data }: { data?: any[] }) => {
    ...
    return (
      <div className="relative p-4 md:p-8 min-w-[800px] overflow-x-auto">
        {data.map((funnel, funnelIndex) => {
          const adGroupsHeight = funnel.adGroups.reduce((acc: number, _: any, i: number) => acc + 50 + (i > 0 ? verticalGap : 0), 0);
          const adsHeight = funnel.ads.reduce((acc: number, _: any, i: number) => acc + 40 + (i > 0 ? verticalGap : 0), 0);
              ...
              {history.length > 0 ? history.map(item => (
                <div key={item.id} className="neumorphic-card flex justify-between items-center p-3">
                  <div>
                    ...
                    <p className="text-xs opacity-60">{new Date(item.id ?? 0).toLocaleString()}</p>
                  </div>
      ...
      <Dialog open={isHistoryModalOpen} onOpenChange={setIsHistoryModalOpen}>
        <DialogContent className="neumorphic-card">
          ...
          <div className="space-y-3 max-h-[60vh] overflow-y-auto p-1">
            {history.length > 0 ? history.map(item => (
              <div key={item.id} className="neumorphic-card flex justify-between items-center p-3">
                <div><p className="font-bold">{item.name}</p><p className="text-xs opacity-60">{new Date(item.id ?? 0).toLocaleString()}</p></div>
