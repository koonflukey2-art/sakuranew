...
export function AccountMenu() {
  ...
  const [isExpanded, setIsExpanded] = useState(() => {
    ...
    const saved = window.localStorage.getItem("account-menu-expanded");
    return saved === null ? true : saved === "true";
  });

  // Expose collapsed state to CSS on <html>
  useEffect(() => {
    if (typeof document === "undefined") return;
    document.documentElement.dataset.accountMenuCollapsed = String(!isExpanded);
  }, [isExpanded]);
  ...
  const toggleExpanded = () => {
    setIsExpanded((prev) => {
      const next = !prev;
      try {
        window.localStorage.setItem("account-menu-expanded", String(next));
      } catch {}
      if (typeof document !== "undefined") {
        document.documentElement.dataset.accountMenuCollapsed = String(!next);
      }
      console.log("[AccountMenu] toggle", { prev, next, collapsed: !next });
      return next;
    });
  };
