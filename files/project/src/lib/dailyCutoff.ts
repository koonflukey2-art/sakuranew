...
export async function createDailySummaryForOrg(
  ...
  orders.forEach((order) => {
    ...
    const orderType = order.productType ?? null;
    const orderName = String(order.productName ?? "").trim();

    const product =
      products.find((p) => orderType != null && p.productType === orderType) ??
      products.find((p) => String(p.name ?? "").trim() === orderName);
    ...
    const key = order.productType != null ? String(order.productType) : (order.productName || "unknown");
