...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!signature) {
      // กัน LINE retry รัว (ถ้า request ไม่ใช่จาก LINE)
    ...
    const valid = verifyLineSig(body, settings.lineChannelSecret, signature);
    if (!valid) {
    ...
    for (const event of events) {
      ...
      await replyLineMessage(
        ...
        `✅ รับออเดอร์แล้ว!\n\n` +
          ...
          `🕐 รอตัดยอดเวลา ${String(settings.dailyCutOffHour).padStart(2, "0")}:${String(
            settings.dailyCutOffMinute
          ).padStart(2, "0")} น.`
      );
      ...
      if (product.quantity < product.minStockLevel && settings.notifyOnLowStock && settings.lineNotifyToken) {
        await sendLineNotify(
          settings.lineNotifyToken,
