...
export default function StockPage() {
  ...
  const fetchProductStats = async () => {
    try {
      ...
      const productsData: any[] = Array.isArray(productsJson)
        ? productsJson
        : Array.isArray(productsJson?.products)
        ? productsJson.products
        : products;

      const stats: Record<number, { sold: number; revenue: number; stock: number }> = {};

      productsData.forEach((product: any) => {
        if (product.productType) {
          const productOrders = ordersData.filter(
            (o: any) => o.productType === product.productType
          );

          stats[product.productType] = {
            sold: productOrders.reduce((sum: number, o: any) => sum + (o.quantity ?? 0), 0),
            revenue: productOrders.reduce((sum: number, o: any) => sum + (o.amount ?? 0), 0),
                        ...
                        >
                          ...
                          <TableCell className="font-medium">
                            <div>
                              ...
                              {product.productType && productStats[product.productType] && (
                                <div className="text-xs text-gray-400 mt-1">
                                  ขายไป: {productStats[product.productType].sold} ชิ้น |
                                  รายได้: ฿{productStats[product.productType].revenue.toLocaleString()}
                                </div>
                              )}
                            </div>
                          </TableCell>
